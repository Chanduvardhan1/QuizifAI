  ...
  useEffect (() =>{
    const fetchQuizData = async () =>{
        try{
            ...
            const response = await fetch(`https://dev.quizifai.com:8010/global_score/`,{
                method: "POST",
                headers: {
  ...
  <div className='text-[12px] text-[#002366] text-nowrap font-semibold mr-[30px] relative top-[25px]'>
    ...
    <p className='text-center pl-1'>{country}</p>
      ...
      <button onClick={handleToggle} className="text-blue-500 text-[12px] underline underline-offset-1 hover:no-underline">
       ...
       <input 
        type="search" 
        className="border-black p-1 border-2 rounded-lg bg-[url('data:image/svg+xml,%3csvg xmlns=%27http://www.w3.org/2000/svg%27 fill=%27none%27 viewBox=%270 0 24 24%27 stroke=%27currentColor%27%3e%3cpath strokeLinecap=%27round%27 strokeLinejoin=%27round%27 strokeWidth=%272%27 d=%27M21 21l-4.35-4.35M16.5 10.5a6 6 0 11-12 0 6 6 0 0112 0z%27 /%3e%3c/svg%3e')] bg-no-repeat bg-left-3 bg-center" 
        placeholder="search"
        value={searchTerm}
