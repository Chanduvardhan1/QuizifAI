...
const qizmasterquizess = () => {
  ...
  useEffect(() => {
    ...
    const fetchQuizzes = async () => {
      try {
        const response = await fetch(
          `https://dev.quizifai.com:8010/quizzes_created_by_user/?user_id=${userId}`,
          {
            method: "GET",
            headers: {
              Accept: "application/json",
        ...
        const result = await response.json();
        console.log("Quizzes data:", result);

        if (result.response === "success") {
            setAllquizzes(result.data || []);
        } else {
          setError(result.response_message || "Failed to fetch quizzes.");
        }
      } catch (err) {
                  ...
                  className={`
                    ${
                      quizItem.active_flag?.toLowerCase() === "i"  || quizItem.active_flag === "false"
                        ? "border-[#A7A7A7] border-[1px] border-b-[8px]": "border-[#84acfa] border-[1px] border-b-[8px]"
                    }
...
export default qizmasterquizess;
