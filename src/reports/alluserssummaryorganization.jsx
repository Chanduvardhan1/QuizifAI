...
const alluserssummaryorganization= () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(
          `https://dev.quizifai.com:8010/all-users-summary-report-for-organization-admin/?org_id=${orgId}`,
          {
            method: "GET",
            headers: {
        ...
        if (result.response === "success") {
          ...
          const uniqueOrganizations = [
            ...new Set(result.data.map((user) => user.organization_name)),
          ];
          const uniqueRoles = [...new Set(result.data.map((user) => user.user_role))];
          const uniqueUserTypes = [...new Set(result.data.map((user) => user.user_type))];
  ...
  // Filter users when search or dropdown changes
  useEffect(() => {
    const filtered = users.filter((user) => {
      return (
        (organizationFilter === "" || user.organization_name === organizationFilter) &&
        (roleFilter === "" || user.user_role === roleFilter) &&
        (userTypeFilter === "" || user.user_type === userTypeFilter) &&
        (searchQuery === "" ||
  ...
  }, [searchQuery, organizationFilter, roleFilter, userTypeFilter, users]);
...
<DashBoardNavBar/>
          ...
          {/* <div className="py-[20px] my-[10px]">
            ...
            <div className="flex gap-[5px] justify-between items-center pl-[5px] mb-4">
...
<div className="flex w-full gap-2 ">
        ...
        {/* User Type Filter */}
        <span className="text-[#F17530] text-[14px]">User Type :</span>
        <select
          className="py-1 px-2 text-[12px] rounded-md border"
        ...
        <thead className="bg-[#CBF2FB]">
          ...
          <th className="py-2 px-4 border-b">Org ID</th>
            ...
            <th className="py-2 px-4 border-b text-start">Organization</th>
            <th className="py-2 px-4 border-b">Role</th>
            <th className="py-2 px-4 border-b">User Type</th>
            <th className="py-2 px-4 border-b">Newly Registered Users</th>
          </tr>
        </thead>
        <tbody>
          {filteredUsers.map((user, index) => (
            ...
            >
              ...
              <td className="py-2 px-4 border-b text-center">
                {`${user.registration_year}-${user.registration_month.trim()}-${user.registration_day}`}
              </td>
              ...
              <td className="py-2 px-4 border-b text-center">
                {user.newly_registered_users_count}
              </td>
            </tr>
          ))}
        </tbody>
...
export default alluserssummaryorganization;
