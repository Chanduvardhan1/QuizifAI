...
const organizationtopscore = () => {
  ...
  useEffect(() => {
    ...
    fetch(`https://dev.quizifai.com:8010/organization-top-scorers-for-org-admin/?org_id=${orgId}`, {
      method: "GET",
      headers: {
  ...
  const uniqueUserNames = [...new Set(users.map(user => user.user_name))];
  const uniqueClassSections = [...new Set(users.map(user => user.class_section))];
  
  // Filter users based on search and dropdown selections
  const filteredUsers = users.filter(user =>
    user.user_name.toLowerCase().includes(searchTerm.toLowerCase()) &&
    (selectedUserName ? user.user_name === selectedUserName : true) &&
    (selectedClassSection ? user.class_section === selectedClassSection : true)
  );
...
<div  className="flex w-full gap-2 ">
      ...
      <div>
        ...
        >
          <option value="">All</option>
          {uniqueClassSections.map((section) => (
      ...
      <thead className="bg-[#CBF2FB]">
        <tr className="text-[14px]">
          ...
          <th className="py-2 px-4 border-b text-center">Total Quizzes Attempted</th>
          <th className="py-2 px-4 border-b text-nowrap">Average Score</th>
          <th className="py-2 px-4 border-b text-center">Total Score</th>
      ...
      <tbody>
        {filteredUsers.map((user, index) => (
          <tr key={user.user_id} className="bg-white hover:bg-gray-100 active:bg-green-200 text-[12px]">
            ...
            <td className="py-2 px-4 border-b text-center">{user.total_quizzes_attempted}</td>
            <td className="py-2 px-4 border-b text-center">{user.average_score}</td>
            <td className="py-2 px-4 border-b text-center">{user.total_score}</td>
            <td className="py-2 px-4 border-b text-center">{user.rank}</td>
          </tr>
        ))}
      </tbody>
    </table>
...
export default organizationtopscore;
