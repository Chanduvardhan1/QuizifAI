...
const [quizMasterNames, setQuizMasterNames] = useState([]);
...
useEffect(() => {
  ...
  fetch(`https://dev.quizifai.com:8010/quiz-master-performance-for-org-admin/?org_id=${orgId}`, {
    method: "GET",
    headers: {
    ...
    .then((data) => {
      ...
      setQuizMasterNames([...new Set(data.data.map((item) => item.quiz_master_name))]); // Extract unique names
    })
    .catch((error) => console.error("Error fetching data:", error));
}, []);

// Filtered Data
const filteredQuizMasters = quizMasters.filter(
  (quizMaster) =>
    quizMaster.quiz_master_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    quizMaster.total_quizzes_created.toString().includes(searchTerm) ||
    quizMaster.average_quiz_complexity.toString().includes(searchTerm) ||
    quizMaster.total_attempts_on_quizzes.toString().includes(searchTerm)
);
...
<DashBoardNavBar/>
      ...
      <select className="py-1 px-2 text-[12px] rounded-md border ml-2">
        ...
        <input
          type="search"
          className="p-1 border-2 border-black rounded-lg bg-no-repeat bg-left-3 bg-center"
          placeholder="Search"
          value={searchTerm}
      ...
      <thead className="bg-[#CBF2FB]">
        <tr className="text-[14px]">
          ...
          <th className="py-2 px-4 border-b text-start">Quiz Master Name</th>
          <th className="py-2 px-4 border-b">Total Quizzes Created</th>
          <th className="py-2 px-4 border-b text-center">Avg Quiz Complexity</th>
      ...
      <tbody className="space-y-4">
        {filteredQuizMasters.map((quizMaster, index) => (
          <tr key={quizMaster.quiz_master_id} className="bg-white hover:bg-gray-100 text-[12px]">
            <td className="py-2 px-4 border-b text-center">{index + 1}</td>
            <td className="py-2 px-2 border-b text-center">{quizMaster.quiz_master_id}</td>
            <td className="py-2 px-4 border-b text-start">{quizMaster.quiz_master_name}</td>
...
export default organizationquizmaster;
